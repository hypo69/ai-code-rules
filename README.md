...
1. **Запрет в аргументах:** Аргументы функций MUST NOT принимать `None` в качестве значения по умолчанию.
2. **Запрет в инициализации:** Локальные, глобальные переменные и свойства классов MUST NOT инициализироваться значением `None`.
3. **Запрет в проверках (is None):** Операторы сравнения `is None` и `is not None` КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ. Проверка состояния объектов должна выполняться на предмет их истинности (boolean truthiness) или через явную валидацию значений по умолчанию.
4. **Запрет в возвратах:** Функции MUST NOT возвращать `None`. В случае сбоя возвращается строго `False`.

#### Альтернативные дефолтные значения (вместо `None`):
Вместо `None` всегда MUST использоваться пустые (дефолтные) значения соответствующих типов данных:
