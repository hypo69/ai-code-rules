...
* **SHOULD (СЛЕДУЕТ / РЕКОМЕНДУЕТСЯ)**: Означает, что в конкрестных обстоятельствах могут существовать веские причины для отклонения от правила, но разработчик (или ИИ) обязан полностью оценить последствия такого выбора, задокументировать его обоснование и взвесить все риски.
...
### 3.6 Запрет использования `None` в качестве аргумента по умолчанию и инициализации
Использование значения `None` в качестве значения по умолчанию для аргументов функций или в процессе инициализации переменных **MUST NOT** допускаться из-за неявности типа данных. Вместо `None` всегда MUST использоваться пустые (дефолтные) значения соответствующих типов данных:
...
* [ ] Использование значения `None` в качестве аргументов по умолчанию или при инициализации переменных полностью исключено. Все переменные инициализируются дефолтными пустыми значениями соответствующих типов (`0`, `''`, `[]`, `{}`).
