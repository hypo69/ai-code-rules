...
* **Плохо:** `// Отправляет запрос`, `// Суммаризирует текст`, `// Объединям данные`
...
Поскольку эти вспомогательные утилиты внутри себя обрабатывают все исключения файловой системы, выполняют обязательное логирование и возвращают значение False в случае сбоя, **исключения не будут передаваться** вызывающему коду.
