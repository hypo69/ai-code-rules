...
Оборачивание вызовов этих утилит в блоки `try/except` является логически избыточным и **НЕ ДОПУСКАЕТСЯ** (MUST NOT). Вместо этого вызывающий код должен явно проверять результат выполнения, контролируя, не вернула ли функция ложное значение.

```python
# ✅ Правильно: Явная проверка возвращаемого значения без лишнего перехвата ошибок
# Примечание: try/except не требуется, так как j_loads обрабатывает и логирует исключения внутри.
config_data = j_loads(config_path)
if not config_data:
    logger.error(f"Не удалось загрузить файл конфигурации: {config_path}")
    return None

# ❌ Неправильно: Логически избыточное оборачивание системной функции в try/except
# Примечание: Блок except никогда не выполнится, так как j_loads перехватит все ошибки сам.
try:
    config_data = j_loads(config_path)
except Exception as e:
    logger.error("Ошибка файловой системы", e)  # Избыточный мертвый код
```
