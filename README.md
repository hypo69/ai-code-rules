...
Настоящий документ определяет **Единый инженерный стандарт разработки** (Engineering Standard v1.0) для экосистемы интеллектуального помощника. Стандарт спроектирован как руководящий технический регламент для разработчиков и как системная инструкция для ИИ-ассистентов (LLM), участвующих в написании, рефакторинге и аудите кодовой базы [1].

### 1.1 Область действия
Требования стандарта распространяются на:
* Все новые и существующие исходные файлы во всех репозиториях проекта [1].
* Архитектурные решения, структуру каталогов, форматы хранения конфигураций и секретов.
* Форматирование кода, структуру комментариев и генерацию автодокументации [1].
...
#### Запрет на использование `None` и неявного `return`
Для исключения двусмысленности и повышения строгости типов данных функции **MUST NOT** возвращать значение `None` или использовать пустой оператор `return` (который неявно возвращает `None`).

При обработке ошибочных сценариев, отсутствии данных, раннем выходе или сбое функции ОБЯЗАНЫ явно возвращать `False` (или `false` для JS/PHP/HTML).
...
Поскольку эти вспомогательные утилиты внутри себя обрабатывают все исключения файловой системы, выполняют обязательное логирование и возвращают ложное значение (falsy, например, `False` или `None`) в случае сбоя, **исключения не будут передаваться** вызывающему коду.
...
# Примечание: Утилита j_loads сама логирует ошибку внутри себя при сбое. 
# Вызывающий код просто выполняет ранний возврат (Early Return) без повторного логирования.
config_data = j_loads(config_path)
if not config_data:
    return False

# ❌ Неправильно: Избыточное логирование (дублирование логов) и неверный try/except
# Примечание: Исключение не возникнет, а повторный вызов логгера засоряет логи.
try:
    config_data = j_loads(config_path)
    if not config_data:
    ...
    logger.error("Ошибка файловой системы", e)  # Мертвый код, блок никогда не выполнится
```
