...
1. Операторы сравнения `is None` и `is not None` КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ. Проверка состояния объектов должна выполняться на предмет их истинности (boolean truthiness) или через явную валидацию значений по умолчанию.
2. Функции **MUST NOT** возвращать `None`. В случае сбоя возвращается строго `False`.
...
Все файловые операции in Python MUST выполняться только через специализированные функции-обертки из библиотеки утилит проекта:
