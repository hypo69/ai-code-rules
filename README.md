...
Операторы сравнения `is None` и `is not None` **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ** (MUST NOT). Проверка состояния объектов должна выполняться на предмет их истинности (boolean truthiness) или через явную валидацию значений по умолчанию.
