...
Для англоязычных комментариев (весь Web-стек и PHP) SHOULD использоваться существительные или отглагольные формы (Noun/Gerund), например: `Initialization`, `Verification`, `Loading`, `Execution` (вместо `Initializes`, `Verifies`, `Loads`).

### 5.3 Языковые зоны комментариев
Для предотвращения смешения языков в проекте действуют строгие географические зоны комментирования:

| Тип расширения файлов | Комментарии в коде | Документирование (Docstrings / JSDoc) | Заголовки (Header Blocks) |
| :--- | :--- | :--- | :--- |
| `.js`, `.ts`, `.jsx`, `.tsx` | **English only** | **English only** | **English only** |
| `.css`, `.scss`, `.sass` | **English only** | **English only** | **English only** |
| `.html` | **English only** | — | **English only** |
| `.php` | **English only** | **English only** | **English only** |
| `.py`, `.ps1` | **Russian / English** | **Russian / English** | **Russian / English** |
